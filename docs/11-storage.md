...
## 6. SQLite — 레코드는 아직 JSONL 이다

**두 저장소를 갈라 둔다.**

| | 무엇 | 형식 |
|---|---|---|
| **data plane** | `data/shared/records/*.jsonl` · `data/camps/<id>/…` | JSONL. 분석 코퍼스 |
| **control plane** | `data/control.db` | SQLite. 계정·세션·승인·감사 (P-002) |

`docs/00-overview.md §4-1` 이 "원본 JSONL(불변) + SQLite" 를 예고했지만, 그 SQLite 는
**레코드 질의 계층**을 가리킨다. 그건 아직 없다. 지금 있는 `control.db` 는 다른 것이다 —
목적도 수명도 다르고, 예고된 `data/votelink.db` 라는 이름을 일부러 쓰지 않았다.

### control plane 은 왜 먼저 왔나

이 문서는 SQLite 도입 트리거를 "건수가 자릿수로 커질 때"로 적어뒀다. 인증은 **양이 아니라
동시성**으로 그 시점을 앞당겼다 — 동시 로그인·세션 갱신·만료 정리는 파일 락으로 감당할
일이 아니다. 그렇다고 레코드까지 옮기지 않는다.

연결을 전역에 들지 않는다(`control/db.py:connect`). `PRAGMA foreign_keys` 는 연결마다 꺼진
채로 시작하므로 매번 켜고, `journal_mode=WAL` 로 운영자 화면과 캠프 열람이 겹쳐도 막히지
않게 한다. **`audit_log.account_id` 에는 외래키가 없다** — 감사 로그는 이력이고, 이력이 현재
상태에 묶이면 계정이 지워질 때 "유출이 없었다"를 증명할 표가 함께 사라진다.

### 레코드 질의 계층이 필요해지는 시점

건수가 자릿수로 커질 때다 — `foot_traffic`(시간대별 유동인구)처럼 행이 동 × 시간대 × 날짜로
곱해지는 kind 가 들어오면 JSONL 전량 로드가 먼저 무너진다. 지금은 L2 분석기도 L3 웹앱도
전량을 메모리에 올리며 그게 가장 단순하다
