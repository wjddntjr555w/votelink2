...
```bash
uv run votelink collect nec_archive --dry-run   # 저장 없이 계약 검증만
uv run votelink collect nec_archive             # 실제 수집
```

**필요한 것이 없다.** 인증키도, 네트워크도, 다운로드도 없다.
`openpyxl`/`xlrd` 는 `uv sync` 가 알아서 깐다.

### 4-3. ✅ 검증 완료 (2026-09-02)

배치 8 · 파싱 72 · 유효 72 · **격리 0** · `verified: true`

송파갑 9동 × 대선 8회. `geo_code` 가 `mois_population` 과 정확히 일치해
L2 에서 개표결과 × 인구 조인이 성립한다.

### 4-4. 응답이 예상과 다르면

33년치가 서로 다른 레이아웃이라 **선거별 레이아웃 프로파일**을 `meta.yaml` 에 뒀다.

| 증상 | 고칠 곳 |
|---|---|
| 특정 선거만 0건 | `elections[].layout` 의 열 인덱스, 특히 `precinct` 와 `precinct_total_markers` |
| `득표 합이 출처의 '계'와 다르다` | `candidate_from` / `total` 열 경계 |
| `집계 항목이 후보 열에 들어왔다` | `total` 이 너무 큼 (계·무효 열까지 후보로 잡음) |
| 새 선거 추가 | `elections` 에 한 항목. **코드는 건드리지 않는다** |

> 새 선거를 붙일 때는 먼저 헤더 몇 줄을 눈으로 보고 열 인덱스를 세는 게 빠르다.
> 선거마다 A열이 비어 있거나(18대) 제목행이 있거나(21대) 한다.

---

## 4-x. ~~선관위 개표결과 CSV~~ (은퇴함)

> **`nec_election_result` 는 2026-09-02 에 은퇴했다.** `nec_archive` 가 같은
> 데이터를 8배 넓은 기간으로 가져오고, 둘을 함께 두면 21대 대선이 두 번 저장돼
> L2 에서 이중 계상된다. 사유는 `collectors/_retired/README.md`.
>
> 아래는 기록으로 남긴다. 다시 쓸 일은 없다.

과거 선거 개표결과를 받던 `nec_election_result` 수집기가 썼다.
