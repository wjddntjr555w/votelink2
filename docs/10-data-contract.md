...
| 기관 | 코드 체계 |
|---|---|
| 행정안전부 | 행정동코드 10자리 / 법정동코드 10자리 (**서로 다른 체계다**) |
| 행정표준코드관리시스템 | 행정기관코드 7자리 |
| 통계청(KOSIS) | 통계용 행정구역 코드 8자리 |
| 중앙선관위 | 자체 선거구·투표구 코드 (개표결과 파일은 코드 없이 **이름만** 준다) |

**결정: 내부 표준은 행정안전부 `행정동코드` 10자리다.**
(예: 서울 송파구 풍납1동 = `1171051000`)
행안부 주민등록 인구 API 의 `admmCd` 가 이 체계이며, 실제 응답으로 확인된 값이다.

**단일 진실은 `votelink/contract/models.py` 의 `GEO_CODE_DIGITS` 상수**다.
...
|---|---|---|
| `nation` | 없음 (null) | |
| `sido` / `sigungu` / `emd` | 숫자 **10자리** | 계층은 자릿수가 아니라 **접두사**로 인코딩된다 |
| `point` | 숫자 10자리 | **지점도 '포함하는 행정동' 코드를 갖는다.** 좌표는 payload에 |
| `none` | 없음 (null) | 전략에 쓸 수 없는 데이터 |

모든 레벨이 10자리이고 레벨은 뒤쪽 0의 개수로 드러난다.

```
서울특별시   1100000000
송파구       1171000000     ← 앞 5자리가 같다
풍납1동      1171051000
```

계층이 접두사에 들어 있어 **상위 단위 집계가 문자열 prefix 매칭으로 된다.**
뉴스는 `sigungu`, 인구·개표결과는 `emd` 로 들어와도 `11710` 으로 묶인다.

**자릿수 검증은 약한 방어선이다.** 법정동코드도 10자리라서 자릿수로는 걸러지지 않는다
(풍납동 법정동코드 `1171010300` ≠ 풍납1동 행정동코드 `1171051000` — 자릿수가 같고
앞 4자리까지 같은데 값이 다르다). 진짜 검증은 "그 코드가 출처 응답이나 매핑표에
실재하는가"이며, 그건 수집기가 책임진다.

법정동코드(10자리), 행정기관코드(7자리), 통계청 행정구역코드(8자리)는
**내부 표준이 아니다.** 매핑표의 `source_code` 로만 존재하며
`geo_code` 에 그대로 들어가면 거부된다. 행정기관코드는 `districts.yaml` 에
