...
15. **자동 주간 브리핑 문장 (텍스트 트랙)** — **검토 결과: 보류 (2026-09-15), 생각보다 범위가 크다.** 6번의 숫자 요약을 자연어로 바꾸는 것뿐이지만 확인해보니 세 가지가 막혀 있다: ① Anthropic API 키가 아직 없다(`docs/SETUP.md §6` "앞으로 필요해질 것 (아직 아님)"). ② **지금 있는 L2 프레임워크 자체가 텍스트 트랙을 못 담는다** — `votelink/analyze/base.py`의 `BaseAnalyzer`는 `load()`(디스크만)·`compute()`(순수 함수, 네트워크·LLM 명시적 금지) 두 단계뿐이라, Claude API를 부르려면 L1의 `fetch()`처럼 네트워크가 허용되는 새 실행 단계를 프레임워크에 추가하는 설계 변경이 먼저 필요하다 — "분석기 하나 추가"가 아니라 "텍스트 트랙을 프로젝트에 처음 들이는" 작업. ③ `docs/00-overview.md`가 이미 이걸 "v0.2로 미룸"에 넣어뒀다(일정표 최적화·메시지 생성·게시물 배치와 같이). 사용자 확인대로 지금은 보류 — v0.1 범위(수치 트랙)를 마저 다지고, 텍스트 트랙은 API 키 발급·프레임워크 확장 설계를 포함해 별도 세션에서 처음부터 설계한다.
