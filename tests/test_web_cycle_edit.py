...
def preview(client, cycle_id=CURRENT, **over):
    return client.post(f"/cycles/{cycle_id}/edit", data=form_for(**over), follow_redirects=False)


def apply(client, cycle_id=CURRENT, **over):
    return client.post(f"/cycles/{cycle_id}/apply", data=form_for(**over), follow_redirects=False)
...
def test_the_preview_changes_nothing_on_disk(env):
    ...
    assert "이대로 저장할까요" in response.text
...
def test_the_preview_names_the_dongs_that_come_and_go(env):
    ...
    client = camp_client(env, "gap@test", "gap", "test_gap")
    html = preview(client, preset="test_eul").text

    assert "추가되는 동 3개" in html
    assert "빠지는 동 3개" in html
    assert "동1000" in html  # test_gap 의 첫 동 — 빠지는 쪽
    assert "동1000" in html and "1171051000" in html, "이름과 코드를 함께 보여준다"
...
def test_the_preview_warns_about_districts_that_close(env):
    ...
    assert "더 이상 볼 수 없게 되는 선거구" in html
    assert "시험 지역구 갑" in html
    assert "시험 지역구 을" in html  # 열리는 쪽


def test_the_preview_warns_that_the_lineage_flips_the_screen(env):
    client = camp_client(env, "gap@test", "gap", "test_gap")
    html = preview(client, lineage="conservative").text
    assert "우세·열세가 뒤집힙니다" in html


def test_the_preview_warns_that_the_folder_moves(env):
    """`cycle_id` 는 선거일과 계열에서 나온 파생값이다."""
    client = camp_client(env, "gap@test", "gap", "test_gap")
    html = preview(client, election_date="2028-04-19").text

    assert "주기 폴더가 옮겨집니다" in html
    assert "2028-04-19-national_assembly" in html


def test_no_change_skips_the_preview(env):
    """바뀌는 것이 없으면 확인을 물을 이유가 없다."""
    client = camp_client(env, "gap@test", "gap", "test_gap")
    response = preview(client)
    assert response.status_code == 303
    assert response.headers["location"] == "/cycles"
...
def test_a_bad_value_goes_back_to_the_form_not_the_preview(env):
    ...
    assert "이대로 저장할까요" not in response.text
...
def test_what_the_preview_promised_is_what_happens(env):
    ...
    client = camp_client(env, "gap@test", "gap", "test_gap")
    html = preview(client, preset="test_eul").text
    assert "시험 지역구 을" in html.split("열리는 선거구")[-1] or "시험 지역구 을" in html
...
def test_the_form_is_prefilled_with_what_is_saved(env):
    ...
    assert 'value="2028-04-12"' in html
    assert "1171051000" in html, "지금 관할이 그대로 들어 있다"
...
def test_the_roster_form_shows_what_is_saved(env):
    ...
    html = client.get(f"/cycles/{CURRENT}/roster").text
    assert 'value="갑후보"' in html
    assert 'value="시험당"' in html


def test_opponents_are_one_per_line(env):
    client = camp_client(env, "gap@test", "gap", "test_gap", name="갑후보")
    response = client.post(
        f"/cycles/{CURRENT}/roster",
        data={
            "ours_name": "갑후보",
            "ours_party": "시험당",
...
def test_an_unknown_lineage_stops_the_line(env):
    ...
    assert response.status_code == 400
    assert "모르는 진영이다 — 우파" in response.text
...
def test_the_roster_round_trips(env):
    ...
    html = client.get(f"/cycles/{CURRENT}/roster").text
    assert "김철수 | 국민의힘 | conservative | 현직" in html
    assert "이영희 | 무소속 | centrist" in html
...
def test_the_roster_list_shows_the_opponent_count(env):
    client = camp_client(env, "gap@test", "gap", "test_gap")
    assert "상대 미등록" in client.get("/cycles").text
    ...
    assert "상대 1명" in client.get("/cycles").text
